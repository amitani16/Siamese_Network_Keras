...
###########################################################################
##### 評価関数および最適化方法の設定 ###########################################
EPOCHS, BATCH_SIZE, Optrate = 300, 1, 0.001
...
img_0_dummy = img_0_dummy.reshape(2, 784) # (r, c)
...
for i in range(EPOCHS):
    ...
    XL_train, yL_train = shuffle(X['train'], y['train']) # print('X_train.shape = ', np.asarray(X_train).shape) # (2, 784) (r,c)

    # Evaluate
    eval_acc  = sess.run(y_conv, feed_dict = {x:  X_train,     y_:  y_train,
                                              xL: img_0_dummy, yL_: dummy_0,
                                              diss: distance_labels_0, keep_prob: 1, })
    eval_1acc = sess.run(y_conv, feed_dict = {x:  X_train,     y_:  y_train,
                                              xL: img_1_dummy, yL_: dummy_1,
                                              diss: distance_labels_1, keep_prob: 1, })
    ...
    for OFFSET in range(0, len(X['train']), BATCH_SIZE):
        ...
        train_step.run(feed_dict = {x:  batch_x,  y_:  batch_y,
                                    ...
                                    diss: distance_labels, keep_prob: 0.5, })
