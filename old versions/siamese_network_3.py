...
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob) # テスト時にはドロップアウトは行わないようにします。（「1.0」を設定）

# Dropoutの設定
hL_fc1_drop = tf.nn.dropout(hL_fc1, keep_prob) # テスト時にはドロップアウトは行わないようにします。（「1.0」を設定）

# Distance
diss_l = tf.abs(tf.subtract(hL_fc1_drop, h_fc1_drop)) # h_fc1_drop(out_ch) = 32
#print('diss_l.shape = ', diss_l.shape)
...
# axis = 1 means returning max in rows
# y_conv: softmax after distance calc
...
cross_entropy = -tf.reduce_sum( diss * tf.log(tf.cast(y_conv, dtype = tf.float32) + (1e-7)) ) + sum_acc +\
...
# 0かどうかの判定
dummy_0 = np.asarray([1.0, 0.0] * 2) # [1, 0, 1, 0]
dummy_0 = dummy_0.reshape(2, 2) # [[1, 0], [1, 0]]
distance_labels_0 = [np.sum(x) for x in dummy_0] # sum in [1, 0]

img_0_dummy = np.asarray(list(rep_0_img)  * 2) # rep_0_img is linealized (784) training image
img_0_dummy = img_0_dummy.reshape(2, 784)

# 1かどうかの判定#print("len(X_train) = ", len(X_train))
dummy_1 = np.asarray([0.0, 1.0] * 2) # len(X_train) = 2
dummy_1 = dummy_0.reshape(2, 2)
...
for i in range(EPOCHS):
    ...
    # Evaluate
    eval_acc  = sess.run(y_conv, feed_dict = {x: X_train, y_: y_train, keep_prob: 1, xL: img_0_dummy, yL_: dummy_0, diss: distance_labels_0})
    eval_1acc = sess.run(y_conv, feed_dict = {x: X_train, y_: y_train, keep_prob: 1, xL: img_1_dummy, yL_: dummy_1, diss: distance_labels_1})
    ...
    dummy_t0 = np.asarray([1.0, 0.0] * len(y['test']))
    dummy_t0 = dummy_t0.reshape(len(y['test']), 2)
    img_t0_dummy = np.asarray(list(rep_0_img)  * len(y['test']))
    img_t0_dummy = img_t0_dummy.reshape(len(y['test']), 784)
    distance_labels = [np.sum(x) for x in dummy_t0]

    eval_acc = sess.run(y_conv, feed_dict={x: X['test'], y_: y['test'], keep_prob: 1, xL: img_t0_dummy, yL_: dummy_t0, diss: distance_labels})

    dummy_t1 = np.asarray([0.0, 1.0] * len(X['test']))
    dummy_t1 = dummy_t0.reshape(len(X['test']), 2)
    img_t1_dummy = np.asarray(list(rep_1_img) * len(X['test']))
    img_t1_dummy = img_t1_dummy.reshape(len(X['test']), 784)
    distance_labels = [(1 - np.sum(x)) for x in dummy_t1]

    eval_1acc = sess.run(y_conv, feed_dict={x: X['test'], y_: y['test'], keep_prob: 1, xL: img_t1_dummy, yL_: dummy_t1, diss: distance_labels})
    ...
    for OFFSET in range(0, len(X['train']), BATCH_SIZE):
        ...
        train_step.run(feed_dict={x: batch_x, y_: batch_y,keep_prob:0.5,xL: batch_xL, yL_:batch_yL,diss:distance_labels})
