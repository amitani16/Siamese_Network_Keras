...
for i in range(EPOCHS):
    ...
    #print('distance_labels = ', distance_labels) # [1, 1]

    # Evaluate
    #eval_acc = sess.run(y_conv, feed_dict = {x: X_train, y_: y_train, keep_prob: 1, xL: img_0_dummy, yL_: dummy_0, diss: distance_labels}) # print('eval_acc = ', eval_acc)
    eval_acc = sess.run(y_conv, feed_dict = {x: X_train, y_: y_train, keep_prob: 1,
                                             xL: img_0_dummy, yL_: dummy_0, diss: distance_labels_0}) # print('eval_acc = ', eval_acc)

    # 1かどうかの判定#print("len(X_train) = ", len(X_train))
    dummy_1 = np.asarray([0.0, 1.0] * len(X_train)) # len(X_train) = 2
    dummy_1 = dummy_0.reshape(len(X_train), 2)
    img_1_dummy = np.asarray(list(rep_1_img) * 2)
    img_1_dummy = img_1_dummy.reshape(2, 784)
    distance_labels_1 = [(1 - np.sum(x)) for x in dummy_1] # print('distance_labels = ', distance_labels) # [0, 0]

    # Evaluate
    eval_1acc = sess.run(y_conv, feed_dict = {x: X_train, y_: y_train, keep_prob: 1,
                                              xL: img_1_dummy, yL_: dummy_1, diss: distance_labels_1})
    ...
    eval_acc = sess.run(y_conv, feed_dict={x: X['test'], y_: y['test'], keep_prob: 1,
                                           xL: img_0_dummy, yL_: dummy_0, diss: distance_labels})
    ...
    eval_1acc = sess.run(y_conv, feed_dict={x: X['test'], y_: y['test'], keep_prob: 1,
                                            xL: img_1_dummy, yL_: dummy_1, diss: distance_labels})
