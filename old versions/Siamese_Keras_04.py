...
def get_diff_pair_test_data(img_list, example_size):
    '''
    https://github.com/akshaysharma096/Siamese-Networks/blob/master/Few%20Shot%20Learning%20-%20V1.ipynb
    '''
    nb_example = 100
    labels  = np.random.choice(nb_class, size = 2, replace = False)
    indices = np.random.randint(0, nb_example, size = example_size)

    # each class contains only one example and chosen from different classes.
    # so target = 0
    target = np.zeros(example_size)

    img_0_list = img_list[ (labels[0] * 100):( (labels[0] + 1) * 100 - 1 ) ]
    img_1_list = img_list[ (labels[1] * 100):( (labels[1] + 1) * 100 - 1 ) ]

    selected_img_0_list = []
    selected_img_1_list = []
    for i in indices:
        selected_img_0_list.append(img_0_list[i]) # plt.imshow(img_0_list[i])# plt.subplot(121)
        selected_img_1_list.append(img_1_list[i]) # plt.imshow(img_1_list[i])# plt.subplot(122)# plt.show()

    selected_img_0 = np.asarray(selected_img_0_list).reshape(example_size, IMG_W, IMG_H, IMG_D)
    selected_img_1 = np.asarray(selected_img_1_list).reshape(example_size, IMG_W, IMG_H, IMG_D)
...
if __name__ == '__main__':
    ...
    selected_img, target = get_diff_pair_test_data(img_list = test_img_list, example_size = 2)
    print(target)
