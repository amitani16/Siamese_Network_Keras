...
###########################################################################
##### 評価関数および最適化方法の設定 ###########################################
EPOCHS, BATCH_SIZE, Optrate = 150, 1, 0.001
...
for i in range(EPOCHS):
    ...
    # X_train and y_train contain shuffled two elements
    ...
    predict_result = np.argmin(matome, axis = 1) # return index >>> Best result is min <<<

    y_right = np.asarray(y_train)
    y_right = np.argmax(y_right, axis = 1) # one-hot. best result is max
    acc_result = np.equal(y_right, predict_result) # check if the inference and right answer are same
    ...
    dummy_t0 = np.asarray([1.0, 0.0] * test_len).reshape(test_len, 2)
    dummy_t1 = np.asarray([0.0, 1.0] * test_len).reshape(test_len, 2)

    img_t0 = np.asarray(list(rep_0_img) * test_len).reshape(test_len, 784)
    img_t1 = np.asarray(list(rep_1_img) * test_len).reshape(test_len, 784)

    distance_labels_0 = [     np.sum(x) for x in dummy_t0]
    distance_labels_1 = [(1 - np.sum(x)) for x in dummy_t1]

    eval_acc = sess.run(y_conv, feed_dict = {x: X['test'], y_:  y['test'],
                                             xL: img_t0,   yL_: dummy_t0,
                                             diss: distance_labels_0, keep_prob: 1, })
    eval_1acc = sess.run(y_conv, feed_dict = {x: X['test'], y_:  y['test'],
                                              xL: img_t1,   yL_: dummy_t1,
                                              diss: distance_labels_1, keep_prob: 1, })

    matome = np.concatenate([eval_acc, eval_1acc], axis = 1)
    predict_result = np.argmin(matome, axis = 1)

    y_right = np.argmax(np.asarray(y['test']), axis = 1)
